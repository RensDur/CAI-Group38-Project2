...
def add_agents(builder, condition, task_type, name, eval_type, folder):
    ...
    for team in range(nr_teams):
        ...
        for agent_nr in range(nr_agents):
            if task_type=="official":
                brain = BaselineAgent(slowdown=8, condition=condition, name=name, eval_type=eval_type, folder=folder) # Slowdown makes the agent a bit slower, do not change value during evaluations
                loc = (22,11)
            if task_type=="tutorial":
...
# Create the world
def create_builder(task_type, condition, name, eval_type, folder):
    ...
    add_agents(builder, condition, task_type, name, eval_type, folder)
